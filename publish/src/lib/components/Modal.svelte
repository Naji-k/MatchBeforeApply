...
{#if open}
  ...
  >
    ...
    >
      {#if title}
        <h2
          style="font-size:1.1rem;font-weight:700;color:var(--color-text-primary);margin-bottom:1rem"
        >
          {title}
        </h2>
      {/if}
      <div
        style="color:var(--color-text-muted);margin-bottom:1.5rem;font-size:.95rem;line-height:1.6"
      >
        {@render children?.()}
      </div>
