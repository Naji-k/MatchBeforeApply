...
<div style="max-width:600px;margin:0 auto;padding:2rem 1rem;display:flex;flex-direction:column;gap:1.5rem">
  ...
  {#if $authStore.user}
    <div class="card" style="padding:.75rem">
      ...
      {/if}
      <p style="color:var(--color-text-muted);font-size:.9rem">{$authStore.user.email}</p>
    </div>
  {/if}
  ...
  <div class="card" style="display:flex;flex-direction:column;gap:1rem;padding:.75rem">
    ...
    {#if uploadSuccess}
      <p style="color:var(--color-success);font-size:.875rem">{uploadSuccess}</p>
    {/if}
    ...
    >
      ...
      {:else}
        <p style="color:var(--color-text-muted);margin-bottom:.4rem">Drag & drop your PDF here</p>
        <p style="font-size:.875rem;color:var(--color-text-muted)">or click to browse</p>
      {/if}
    </div>
  ...
  <!-- Logout -->
  <div class="card">
    <button class="btn-danger" style="width:100%;padding:.85rem" onclick={handleLogout}>
      Logout
    </button>
  </div>
</div>
