<script lang="ts">
  ...
  const activeCount = $derived(
    $applicationsStore.items.filter((a) => a.status === 'open' || a.status === 'in_progress').length
  );
  const avgScore = $derived.by(() => {
    const scored = $applicationsStore.items.filter((a) => a.match_breakdown?.overall_score !== undefined);
    if (!scored.length) return null;
    const sum = scored.reduce((acc, a) => acc + (a.match_breakdown!.overall_score * 10), 0);
  ...
  async function onDrop(newStatus: ApplicationStatus) {
    ...
    applicationsStore.update((s) => ({
      ...s,
      items: s.items.map((a) => a.id === id ? { ...a, status: newStatus } : a)
    }));
...
<div style="max-width:1200px;margin:0 auto;padding:1.5rem 1rem">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem">
    ...
    <a href="/applications/new" class="btn-primary" style="padding:.6rem 1.25rem;text-decoration:none;font-size:.9rem">
      + New Application
    </a>
  </div>
  ...
  <div style="display:flex;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap">
    <div class="card" style="flex:1;min-width:130px;padding:1rem 1.25rem">
      <p style="font-size:.75rem;font-weight:600;color:var(--color-text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.35rem">Total</p>
      <p style="font-size:1.6rem;font-weight:800;color:var(--color-text-primary)">{totalCount}</p>
    </div>
    <div class="card" style="flex:1;min-width:130px;padding:1rem 1.25rem">
      <p style="font-size:.75rem;font-weight:600;color:var(--color-text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.35rem">Avg Score</p>
      <p style="font-size:1.6rem;font-weight:800;color:var(--color-text-primary)">{avgScore !== null ? avgScore + '%' : '—'}</p>
    </div>
    <div class="card" style="flex:1;min-width:130px;padding:1rem 1.25rem">
      <p style="font-size:.75rem;font-weight:600;color:var(--color-text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.35rem">Active</p>
      <p style="font-size:1.6rem;font-weight:800;color:var(--color-accent)">{activeCount}</p>
    </div>
  </div>
  ...
  {:else if $applicationsStore.items.length === 0}
    <div class="card" style="text-align:center;padding:4rem 2rem">
      <p style="color:var(--color-text-muted);font-size:1.05rem;margin-bottom:1.5rem">No applications yet.</p>
  ...
  {:else}
    ...
    <div class="mobile-list">
      {#each COLUMNS as col}
        ...
        {#if items.length > 0}
          <div>
            <h2 style="font-size:.85rem;font-weight:700;color:var(--color-text-muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.65rem">
